![nginx latest](https://img.shields.io/badge/nginx-latest-brightgreen.svg)[![build status](https://gitlab.com/sytertzp/rancher-nginx-proxy/badges/master/build.svg)](https://gitlab.com/sytertzp/rancher-nginx-proxy/commits/master)  ![License MIT](https://img.shields.io/badge/license-MIT-blue.svg)   [![Docker Pulls](https://img.shields.io/docker/pulls/syter/rancher-nginx-proxy.svg)](https://hub.docker.com/r/syter/rancher-nginx-proxy/)  [![Docker Automated buil](https://img.shields.io/docker/automated/syter/rancher-nginx-proxy.svg)](https://hub.docker.com/r/syter/rancher-nginx-proxy/)
...
Rancher Active Proxy replace docker-gen by Rancher-gen-rap [sytertzp/rancher-docker-gen](https://gitlab.com/sytertzp/rancher-docker-gen) ( a fork of the also excellent [janeczku/go-rancher-gen](https://github.com/janeczku/go-rancher-gen) adding some more function )
...
Add `https://gitlab.com/sytertzp/rancher-nginx-proxy.git` to your custom catalog list( Rancher > Admin > Settings ).
...
Minimal Params To run it:
    ...
    $ docker run -d -p 80:80  sytertzp/rancher-nginx-proxy
...
Provided your DNS is setup to forward foo.bar.com to the a host running `rancher-nginx-proxy`, the request will be routed to a container with the `rap.host` label set.
...
| `rap.http_listen_ports`    | External Port you want rancher-nginx-proxy to listen to http for this server ( Default : `80` )
| `rap.https_listen_ports`   | External Port you want rancher-nginx-proxy to listen to https for this server ( Default : `443` )
...
If needed you can use rancher-nginx-proxy to listen for different ports.

`docker run -d -p 8081:8081 -p 81:81  sytertzp/rancher-nginx-proxy`
...
Just start Rancher Active Proxy like that : `docker run -d -p 80:80 -e SPECIFIC_HOST=Hostnameofthehost sytertzp/rancher-nginx-proxy`

### Remove Script

Rancher Active Proxy provides an easy script to revoke/delete a certificate.

You can run it : `docker run sytertzp/rancher-nginx-proxy /app/remove DomainCertToRemove`
...
For example, if you have a virtual host named `app.example.com` and you have configured a proxy_cache `my-cache` in another custom file, you could tell it to use a proxy cache as follows:
    ...
    $ docker run -d -p 80:80 -p 443:443 -v /path/to/vhost.d:/etc/nginx/vhost.d:ro sytertzp/rancher-nginx-proxy
...
To set the default host for nginx use the env var `DEFAULT_HOST=foo.bar.com` for example :
    ...
    $ docker run -d -p 80:80 -e DEFAULT_HOST=foo.bar.com sytertzp/rancher-nginx-proxy
...
To enable SSL:
    ...
    $ docker run -d -p 80:80 -p 443:443 -v /path/to/certs:/etc/nginx/certs  sytertzp/rancher-nginx-proxy
...
```Dockerfile
FROM syter/rancher-nginx-proxy
RUN { \
...
Or it can be done by mounting in your custom configuration in your `docker run` command:
    ...
    $ docker run -d -p 80:80 -p 443:443 -v /path/to/my_proxy.conf:/etc/nginx/conf.d/my_proxy.conf:ro sytertzp/rancher-nginx-proxy
...
Automated Gitlab CI is used to build Rancher Active Proxy therefore send any pull request/issues to [Rancher Active Proxy on Gitlab.com](https://gitlab.com/sytertzp/rancher-nginx-proxy/)
